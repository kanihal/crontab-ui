...
- Outlined, color-coded row actions (Run = green, Edit = blue, stderr log = red, stdout log = cyan, last Test Run = amber, Duplicate = gray, Delete = red) with hover tooltips. The three log actions are grouped together, and distinct toggle icons (green pill = running, gray pill = stopped) replace the previous duplicated play icons.
...
- **Long-running execution tracking** — **Test Run** and **Run now** start asynchronously, stream stdout and stderr, survive page refreshes, and report the actual exit result without holding a proxy request open. At most one Test Run or Run now execution can be active; its job row shows a spinner and **Stop** terminates an indefinite run. Test Run output is available from the row's amber history action. Output is capped at 10 MiB per stream and retained for 24 hours (up to 10 recent results).
