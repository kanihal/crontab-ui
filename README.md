...
- **Long-running Test Run** — a **Test Run** button in the Job modal starts the command asynchronously (with configured environment variables prepended), streams stdout and stderr, survives page refreshes, and reports the actual exit result without holding a proxy request open. One test may run at a time; use **Stop** to terminate an indefinite run. Output is capped at 10 MiB per stream and retained for 24 hours (up to 10 recent results).
